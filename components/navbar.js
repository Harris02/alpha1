...
export default function Navbar(){
    ...
    return(
        <nav className="flex flex-row pt-10 bg-transparent">
            ...
            <button style={{display:isLoggedin?'none':'block'}} onClick={function(){router.push('/register')}} className="bg-purple-700 text-white rounded-lg border-4 w-[100px]">Sign Up</button>
            <button style={{display:isLoggedin?'none':'block'}} onClick={function(){router.push('/login')}} className="rounded-lg border-4 text-white  p-1 px-4  mr-10 ml-5">Login</button>
